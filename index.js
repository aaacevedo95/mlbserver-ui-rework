...
const GAMECHANGER_RESPONSE_HEADERS = {
  statusCode: 200,
  headers: {
...
app.listen(port, function (addr) {
  ...
  session.debuglog(
    "multiview server started at http://" +
      addr.replace(":" + port, ":" + multiview_port) +
      multiview_url_path
  );
  if (ffmpegEncoder == defaultEncoder) {
    session.debuglog(
      "warning: multiview is set to use software encoding -- see the readme for how to configure hardware encoding (to reduce multiview CPU usage and improve multiview performance)"
    );
  }
...
app.get("/stream.m3u8", async function (req, res) {
  ...
  try {
    ...
    if (
      urlArray.length == 1 ||
      (session.data.scan_mode == VALID_SCAN_MODES[1] && req.query.team) ||
      (!req.query.team &&
        !req.query.src &&
        !req.query.highlight_src &&
        !req.query.eventURL &&
        !req.query.event &&
        !req.query.gamePk &&
        !req.query.id &&
      ...
      if (req.query.resolution && options.resolution == "best") {
        ...
        options.resolution = session.returnValidItem(
          req.query.resolution,
          VALID_RESOLUTIONS
        );
      }
      options.audio_track = session.returnValidItem(
        req.query.audio_track,
        VALID_AUDIO_TRACKS
      );
      ...
      if (options.pad != VALID_PAD[0]) {
        ...
        options.pad =
          Math.floor(Math.random() * (7200 / SECONDS_PER_SEGMENT)) +
          3600 / SECONDS_PER_SEGMENT;
      }
      ...
      if (req.query.src) {
        ...
        streamURL = await session.getEventStreamURL(
          req.query.event.toUpperCase()
        );
      } else {
        if (req.query.gamePk) {
        ...
        if (!streamURL) {
          ...
          for (var i = 0; i < ALTERNATE_AUDIO_TRACKS.length; i++) {
            if (req.query[ALTERNATE_AUDIO_TRACKS[i]]) {
              options.alternate_audio_tracks[ALTERNATE_AUDIO_TRACKS[i]] =
                req.query[ALTERNATE_AUDIO_TRACKS[i]];
            }
          }
          if (req.query.mediaId) {
            mediaId = req.query.mediaId;
          } else if (req.query.gamePk && req.query.contentId) {
            let mediaInfo = await session.getMediaIdFromContentId(
              gamePk,
              contentId
            );
            if (mediaInfo) {
              ...
              if (mediaInfo.alternateAudioTracks) {
                for (const [key, value] of Object.entries(
                  mediaInfo.alternateAudioTracks
                )) {
            ...
            let mediaInfo = await session.getMediaId(
              decodeURIComponent(req.query.team),
    ...
    if (streamURL) {
      ...
      if (options.alternate_audio_tracks) {
        for (const [key, value] of Object.entries(
          ...
          if (
            ...
            options.audio_track == key
          ) {
            let audioStreamURL = await session.getStreamURL(value);
            let audioPlaylistURL = audioStreamURL.replace(
              /\/(master_radio_complete|master_radio)/g,
              "/48K/48_complete"
            );
      ...
      if (
        options.inning_half != VALID_INNING_HALF[0] ||
        options.inning_number != VALID_INNING_NUMBER[0] ||
        options.skip != VALID_SKIP[0]
      ) {
        if (contentId) {
          ...
          if (skip_type == 4) {
            let new_streamURL = streamURL.replace(
              "master_desktop_complete",
              "master_desktop_complete_gdfp"
            );
            if (new_streamURL == streamURL) {
              new_streamURL = streamURL.replace(
                "master_desktop",
                "master_desktop_gdfp"
              );
            }
            session.debuglog(
              "skipping commercials using gdfp playlist " + new_streamURL
            );
            streamURL = new_streamURL;
          } else {
            await session.getSkipMarkers(
              contentId,
              skip_type,
              options.inning_number,
              options.inning_half
            );
          }
        }
  ...
  var req = function () {
    ...
    requestRetry(streamURL, headers, function (err, response) {
      ...
      if (
        ...
        resolution != VALID_RESOLUTIONS[VALID_RESOLUTIONS.length - 1]
      ) {
        ...
        body
          .map(function (line) {
            ...
            if (line.startsWith("#EXTINF:")) {
              ...
              return (
                "/ts?url=" +
                encodeURIComponent(url.resolve(streamURL, line.trim())) +
                content_protect +
                referer_parameter
              );
            }
            ...
            if (
              line.startsWith("#EXT-X-MEDIA:TYPE=CLOSED-CAPTIONS,") &&
              (audio_track != VALID_AUDIO_TRACKS[0] ||
                audio_track != VALID_AUDIO_TRACKS[1] ||
                resolution == VALID_RESOLUTIONS[VALID_RESOLUTIONS.length - 1])
            ) {
              return;
            }
            ...
            if (line.startsWith("#EXT-X-MEDIA:TYPE=AUDIO")) {
              ...
              if (
                audio_track == VALID_AUDIO_TRACKS[VALID_AUDIO_TRACKS.length - 1]
              ) {
              ...
              if (!line.includes(",URI=")) {
                ...
                if (
                  ...
                  if (
                    ...
                    if (
                      line.includes('NAME="' + audio_track + '"') ||
                      line.includes(
                        'NAME="' +
                          audio_track.substring(0, audio_track.length - 1) +
                          '"'
                      )
                    ) {
                      audio_track_matched = true;
                  ...
                  if (line.match) {
                    ...
                    if (parsed[1]) {
                      newurl =
                        "/playlist?url=" +
                        encodeURIComponent(
                          url.resolve(streamURL, parsed[1].trim())
                        );
                      if (force_vod != VALID_FORCE_VOD[0])
                        newurl += "&force_vod=on";
                      if (inning_half != VALID_INNING_HALF[0])
                        newurl += "&inning_half=" + inning_half;
                      if (inning_number != VALID_INNING_NUMBER[0])
                        newurl += "&inning_number=" + inning_number;
              ...
              if (!audio_track_matched) {
                for (const [key, value] of Object.entries(
                  ...
                  if (
                    ...
                    audio_track_matched = true;
                    audio_output =
                      '#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID="aac",NAME="' +
                      key +
                      '",LANGUAGE="';
            ...
            if (line.startsWith("#EXT-X-STREAM-INF:BANDWIDTH=")) {
              if (
                ...
                if (resolution === VALID_RESOLUTIONS[0]) {
                  ...
                  if (
                    line.indexOf(resolution + ",FRAME-RATE=" + frame_rate) > 0
                  ) {
            ...
            if (
              line.startsWith(
                '#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="subs",NAME="English",DEFAULT=YES,AUTOSELECT=YES,FORCED=NO,LANGUAGE="eng",URI="'
              )
            ) {
              var parsed = line.match(',URI="([^"]+)"');
              if (parsed[1]) {
                newurl =
                  "/playlist?url=" +
                  encodeURIComponent(url.resolve(streamURL, parsed[1].trim()));
                return (
                  '#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="subs",NAME="English",DEFAULT=YES,AUTOSELECT=YES,FORCED=NO,LANGUAGE="eng",URI="' +
                  newurl +
                  '"'
    ...
    requestRetry(u, headers, function (err, response) {
      ...
      if (skip == "commercials") {
        ...
        (inning_half != VALID_INNING_HALF[0] ||
          ...
          skip != VALID_SKIP[0]) &&
        typeof session.temp_cache[contentId] !== "undefined" &&
        typeof session.temp_cache[contentId].skip_markers !== "undefined"
      ) {
          ...
          .map(function (line) {
            ...
            if (skip_markers && skip_markers[skip_marker_index]) {
              ...
              if (line.startsWith("#EXTINF:")) {
                ...
                while (
                  skip_markers[skip_marker_index] &&
                  skip_markers[skip_marker_index].break_end < time_counter
                ) {
                  skip_marker_index++;
                }
                if (
                  skip_markers[skip_marker_index] &&
                  time_counter >= skip_markers[skip_marker_index].break_start &&
                  time_counter < skip_markers[skip_marker_index].break_end
                ) {
            ...
            let newline =
              "/ts?url=" +
              encodeURIComponent(url.resolve(u, line.trim())) +
              content_protect +
              referer_parameter;
            if (key)
              newline +=
      ...
      if (pad != VALID_PAD[0]) {
        ...
        if (body_array[last_segment_index] == "#EXT-X-ENDLIST") {
          ...
          last_segment_index--;
          while (
            !body_array[last_segment_index].startsWith(
...
app.get("/gamechanger.m3u8", async function (req, res) {
  ...
  if (req.query.resolution && req.query.resolution == "best") {
    ...
    resolution = session.returnValidItem(
      req.query.resolution,
      VALID_RESOLUTIONS
    );
  }
  ...
  for (gamechanger_resolution in GAMECHANGER_RESOLUTIONS) {
    if (resolution == gamechanger_resolution) {
      body +=
        ...
        GAMECHANGER_RESOLUTIONS[gamechanger_resolution].bandwidth +
        "000,RESOLUTION=" +
        GAMECHANGER_RESOLUTIONS[gamechanger_resolution].resolution +
        ",FRAME-RATE=" +
        GAMECHANGER_RESOLUTIONS[gamechanger_resolution].frame_rate +
        ',CODECS="mp4a.40.2,avc1.' +
        GAMECHANGER_RESOLUTIONS[gamechanger_resolution].codec +
        '",CLOSED-CAPTIONS="cc",AUDIO="aac"' +
        "\n" +
        "/gamechangerplaylist?id=" +
        id +
        "&resolution=" +
        gamechanger_resolution +
        includeTeams +
        excludeTeams +
        content_protect +
        "\n";
...
app.get("/gamechangerplaylist", async function (req, res) {
  ...
  if (!id) {
    ...
    var req = async function () {
      ...
      if (
        !session.temp_cache.gamechanger ||
        !session.temp_cache.gamechanger[id] ||
        !session.temp_cache.gamechanger[id].segments ||
        session.temp_cache.gamechanger[id].segments.length == 0 ||
        !session.temp_cache.gamechanger[id].lastAccess ||
        gamechangerAccess >=
          new Date(
            new Date(session.temp_cache.gamechanger[id].lastAccess).getTime() +
              30000
          )
      ) {
      ...
      if (
        !session.temp_cache.gamechanger.start ||
        gamechangerAccess < session.temp_cache.gamechanger.start ||
        !session.temp_cache.gamechanger.end ||
        gamechangerAccess > session.temp_cache.gamechanger.end
      ) {
        session.log(
          game_changer_title + "outside of games starting/ending, skipping"
        );
        respond(GAMECHANGER_RESPONSE_HEADERS, res, Buffer.from(""));
      } else {
        ...
        if (
          ...
          if (
            session.temp_cache.gamechanger[id].segments.length ==
            GAMECHANGER_LIST_SIZE
          ) {
            discontinuity = true;
        ...
        if (!streamURL) {
          ...
          if (
            !session.temp_cache.gamechanger[id].playlist[resolution] ||
            !session.temp_cache.gamechanger[id].lastAccess[resolution] ||
            gamechangerAccess >=
              new Date(
                new Date(
                  session.temp_cache.gamechanger[id].lastAccess[resolution]
            ...
            let u =
              streamURL +
              "/" +
              GAMECHANGER_RESOLUTIONS[resolution].url_bandwidth +
              "K/" +
              GAMECHANGER_RESOLUTIONS[resolution].url_bandwidth +
              "_complete.m3u8";
            requestRetry(u, headers, function (err, response) {
              ...
              if (body[0] != "#EXTM3U") {
                session.log(
                  game_changer_title + "not a valid hls stream at " + u
                );
                return;
              }
              ...
              for (var i = body.length - 1; i >= 0; i--) {
                if (body[i].startsWith("#EXTINF:")) {
                  ...
                  if (!new_segments_complete) {
                    session.debuglog(
                      game_changer_title + "found segment " + line
                    );
                    if (discontinuity) {
                      session.debuglog(
                        game_changer_title +
                          "only getting newest segment after stream change"
                      );
                      ...
                      line ==
                        session.temp_cache.gamechanger[id].segments[
                          session.temp_cache.gamechanger[id].segments.length - 1
                        ].ts
                    ) {
                      session.debuglog(
                        game_changer_title + "found previous last segment"
                      );
                      new_segments_complete = true;
                  ...
                  session.debuglog(
                    game_changer_title + "found key : " + body[i]
                  );
                  ...
                  session.debuglog(
                    game_changer_title +
                      "new segments to key: " +
                      JSON.stringify(segments_to_key)
                  );
              ...
              session.debuglog(
                game_changer_title +
                  "new segments : " +
                  JSON.stringify(new_segments)
              );

              // remove stored segments
              while (
                session.temp_cache.gamechanger[id].segments.length >
                GAMECHANGER_LIST_SIZE - new_segments.length
              ) {
              ...
              session.debuglog(
                game_changer_title + "removing old segments complete"
              );

              // append new segments to cache
              session.temp_cache.gamechanger[id].segments =
                session.temp_cache.gamechanger[id].segments.concat(
                  new_segments
                );
              ...
              for (
                var i = 0;
                i < session.temp_cache.gamechanger[id].segments.length;
                i++
              ) {
                if (
                  session.temp_cache.gamechanger[id].segments[i].discontinuity
                ) {
                  session.temp_cache.gamechanger[id].playlist[resolution] +=
                    "#EXT-X-DISCONTINUITY" + "\n";
                }
                session.temp_cache.gamechanger[id].playlist[resolution] +=
                  session.temp_cache.gamechanger[id].segments[i].extinf +
                  "\n" +
                  "/ts?url=" +
                  encodeURIComponent(
                    session.temp_cache.gamechanger[id].segments[i].ts
                  ) +
                  "&key=" +
                  encodeURIComponent(
                    session.temp_cache.gamechanger[id].segments[i].key
                  ) +
                  "&iv=" +
                  encodeURIComponent(
                    session.temp_cache.gamechanger[id].segments[i].iv
                  ) +
                  content_protect +
                  "\n";
              }
            ...
            session.debuglog(
              game_changer_title + "check for new segments complete"
            );
          } else {
...
async function protect(req, res) {
  if (argv.page_username && argv.page_password) {
    if (
      !session.protection.content_protect ||
      !req.query.content_protect ||
      req.query.content_protect != session.protection.content_protect
    ) {
      if (
        !session.protection.content_protect ||
        !req.query.content_protect ||
        !req.query.content_protect[0] ||
        req.query.content_protect[0] != session.protection.content_protect
      ) {
        const reject = () => {
        ...
        if (
          !(username === argv.page_username && password === argv.page_password)
        ) {
          return reject();
        }
      }
...
app.get("/", async function (req, res) {
  try {
    ...
    let server = "http://" + req.headers.host;
    let multiview_server = server.replace(
      ":" + session.data.port,
      ":" + session.data.multiviewPort
    );
    ...
    if (req.query.org) {
      ...
      if (typeof session.getAffiliateTeamIds(org) === "undefined") {
        ...
        team_ids +=
          session.getTeamIds(org) + "," + session.getAffiliateTeamIds(org);
        level = default_org;
      }
      ...
      for (let i = 0; i < session.credentials.fav_teams.length; i++) {
        if (session.credentials.fav_teams[i] != "") {
          ...
          team_ids +=
            "," + session.getAffiliateTeamIds(session.credentials.fav_teams[i]);
        }
      }
    }
    ...
    var cache_data = await session.getDayData(
      gameDate,
      false,
      level_ids,
      team_ids
    ...
    // TODO: starts here
    var body =
      '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="Content-type" content="text/html;charset=UTF-8"> <meta name="viewport" content="width=device-width, initial-scale=1.0"><title>' +
      appname +
      '</title><link rel="icon" href="favicon.svg' +
      content_protect_a +
      '"><style type="text/css">input[type=text],input[type=button]{-webkit-appearance:none;-webkit-border-radius:0}body{width:480px;color:lightgray;background-color:#242424;font-family:Arial,Helvetica,sans-serif;-webkit-text-size-adjust:none}a{color:darkgray}button{color:lightgray;background-color:black}button.default{color:black;background-color:lightgray}table{width:100%;pad}table,th,td{border:1px solid darkgray;border-collapse:collapse}th,td{padding:5px}.tinytext,textarea,input[type="number"]{font-size:.8em}textarea{width:380px}.freegame,.freegame a{color:green}.blackout,.blackout a{text-decoration:line-through}';

    // Highlights CSS
    //max-height:calc(100vh-110px);
    body +=
      ".modal{display:none;position:fixed;z-index:1;padding-top:100px;left:0;top:0;width:100%;height:100%;overflow:auto;-webkit-overflow-scrolling:touch;background-color:rgb(0,0,0);background-color:rgba(0,0,0,0.4)}.modal-content{background-color:#fefefe;margin:auto;padding:10px;border:1px solid #888;width:360px;color:black}#highlights a{color:black}.close{color:black;float:right;font-size:28px;font-weight:bold;}#highlights a:hover,#highlights a:focus,.close:hover,.close:focus{color:gray;text-decoration:none;cursor:pointer;}";
    // Tooltip CSS
    body +=
  ...
  `;
    body += `${style}</style><script type="text/javascript">` + "\n";
    ...
    body +=
      ...
      todayUTCHours +
      " AM - " +
      (YESTERDAY_UTC_HOURS - 4) +
      ' AM EST.</span></span>: <input type="date" id="gameDate" value="' +
      gameDate +
      '"/> ';
    for (var i = 0; i < VALID_DATES.length; i++) {
      body += "<button ";
      if (
        (VALID_DATES[i] == VALID_DATES[0] && gameDate == today) ||
        (VALID_DATES[i] == VALID_DATES[1] && gameDate == yesterday)
      )
        body += 'class="default" ';
      body +=
        "onclick=\"date='" +
    ...
    for (const [key, _] of Object.entries(levels)) {
      body += "<button ";
    ...
    body +=
      ' or <span class="tooltip">Org<span class="tooltiptext">Major league parent organization</span></span>: ';
    body +=
      '<select id="org" onchange="level=\'' +
      default_org +
      "';org=this.value;reload()\">";
    ...
    body +=
      '<p><span class="tooltip">Media Type<span class="tooltiptext">Video is TV broadcasts, Audio is English radio, and Spanish is Spanish radio (not available for all games).</span></span>: ';
    ...
    if (linkType == VALID_LINK_TYPES[0]) {
      body +=
        '<span class="tooltip">Video Controls<span class="tooltiptext">Choose whether to show or hide controls on the embedded video page. Helpful to avoid timeline spoilers.</span></span>: ';
    ...
    if (mediaType == VALID_MEDIA_TYPES[0]) {
      ...
      body += " ";
      body +=
        '<select id="inning_number" onchange="inning_number=this.value;reload()">';
    ...
    body +=
      '<p><span class="tooltip">Scores<span class="tooltiptext">Choose whether to show scores on this web page. Combine this with the inning option to only show scores through the specified inning.</span></span>: ';
    ...
    // TODO: table start
    body += `<div class="flex justify-center flex-wrap">`;
    ...
    if (
      mediaType == "MLBTV" &&
      (level_ids == levels["MLB"] || level_ids.startsWith(levels["MLB"] + ","))
    ) {
      if (
        gameDate >= today &&
        cache_data.dates &&
        cache_data.dates[0] &&
        cache_data.dates[0].games &&
        cache_data.dates[0].games.length > 0
      ) {
        blackouts = await session.get_blackout_games(
          cache_data.dates[0].games,
          true
        );
      }
      ...
      if (
        cache_data.dates &&
        cache_data.dates[0] &&
        cache_data.dates[0].date >= today &&
        cache_data.dates[0].games &&
        cache_data.dates[0].games.length > 1 &&
        cache_data.dates[0].games[0] &&
        cache_data.dates[0].games[0].seriesDescription == "Regular Season"
      ) {
      ...
      if (big_inning && big_inning.start) {
        body +=
          '<div class="w-full m-4"><span><span class="tooltip">' +
          new Date(big_inning.start).toLocaleString("en-US", {
            hour: "numeric",
            minute: "numeric",
            hour12: true,
          }) +
          " - " +
          new Date(big_inning.end).toLocaleString("en-US", {
            hour: "numeric",
            minute: "numeric",
            hour12: true,
          }) +
          '<span class="tooltiptext">Big Inning is the live look-in and highlights show. <a href="https://www.mlb.com/live-stream-games/big-inning">See here for more information</a>.</span></span></td><td>';
        ...
        if (currentDate >= compareStart && currentDate < compareEnd) {
          ...
          let multiviewquerystring =
            querystring + "&resolution=" + DEFAULT_MULTIVIEW_RESOLUTION;
          if (linkType == VALID_LINK_TYPES[0]) {
            if (startFrom != VALID_START_FROM[0])
      ...
      // Game Changer Commenting game changer till i fully understand it
      // if (
      //   gameDate >= today &&
      //   cache_data.dates &&
      //   cache_data.dates[0] &&
      ...
      //   cache_data.dates[0].games.length > 1
      // ) {
      //   let gameIndexes = await session.get_first_and_last_games(
      //     cache_data.dates[0].games,
      //     blackouts
      //   );
      //   if (
      //     typeof gameIndexes.firstGameIndex !== "undefined" &&
      //     typeof gameIndexes.lastGameIndex !== "undefined" &&
      //     gameIndexes.firstGameIndex !== gameIndexes.lastGameIndex
      //   ) {
      //     let compareStart = new Date(
      //       cache_data.dates[0].games[gameIndexes.firstGameIndex].gameDate
      //     );
      //     let compareEnd = new Date(
      //       cache_data.dates[0].games[gameIndexes.lastGameIndex].gameDate
      //     );
      //     if (
      //       cache_data.dates[0].games[gameIndexes.lastGameIndex].status &&
      //       cache_data.dates[0].games[gameIndexes.lastGameIndex].status
      //         .startTimeTBD == true
      //     ) {
      //       compareEnd = new Date(
      //         cache_data.dates[0].games[gameIndexes.lastGameIndex - 1].gameDate
      //       );
      ...
      //     body +=
      //       '<div class="w-full m-4"><span><span class="tooltip">' +
      //       compareStart.toLocaleString("en-US", {
      //         hour: "numeric",
      //         minute: "numeric",
      //         hour12: true,
      //       }) +
      //       " - " +
      //       compareEnd.toLocaleString("en-US", {
      //         hour: "numeric",
      //         minute: "numeric",
      //         hour12: true,
      //       }) +
      ...
      //         '<input type="checkbox" value="' +
      //         multiviewquerystring +
      //         '" onclick="addmultiview(this, [], excludeTeams)">';
      //     } else {
      //       body += "Game Changer";
      //     }
      //     body += "</span></div>" + "\n";
    ...
    if (cache_data.dates && cache_data.dates[0] && cache_data.dates[0].games) {
      for (var j = 0; j < cache_data.dates[0].games.length; j++) {
        ...
        let awayteam =
          cache_data.dates[0].games[j].teams["away"].team.abbreviation;
        let awayteam_abbr;
        if (
          cache_data.dates[0].games[j].teams["away"].team.sport.name !=
          "Major League Baseball"
        ) {
          awayteam =
            cache_data.dates[0].games[j].teams["away"].team.shortName +
            " (" +
            session.getParent(
              cache_data.dates[0].games[j].teams["away"].team.parentOrgName
            ) +
            ")";
          awayteam_abbr =
            cache_data.dates[0].games[j].teams["away"].team.abbreviation;
          awayteam_level = session.getLevelNameFromSportId(
            cache_data.dates[0].games[j].teams["away"].team.sport.id
          );
        }
        let hometeam =
          cache_data.dates[0].games[j].teams["home"].team.abbreviation;
        let hometeam_abbr;
        if (
          cache_data.dates[0].games[j].teams["home"].team.sport.name !=
          "Major League Baseball"
        ) {
          hometeam =
            cache_data.dates[0].games[j].teams["home"].team.shortName +
            " (" +
            session.getParent(
              cache_data.dates[0].games[j].teams["home"].team.parentOrgName
            ) +
            ")";
          hometeam_abbr =
            cache_data.dates[0].games[j].teams["home"].team.abbreviation;
          hometeam_level = session.getLevelNameFromSportId(
            cache_data.dates[0].games[j].teams["home"].team.sport.id
          );
        }
        ...
        if (awayteam_abbr) {
          teams +=
            '<span class="tooltip">' +
            awayteam +
            '<span class="tooltiptext">Team Abbreviation: ' +
            awayteam_abbr +
            ", level " +
            awayteam_level +
            "</span></span>";
        ...
        if (cache_data.dates[0].games[j].status.startTimeTBD == true) {
          ...
          state += startTime.toLocaleString("en-US", {
            hour: "numeric",
            minute: "numeric",
            hour12: true,
          });
        ...
        var abstractGameState =
          cache_data.dates[0].games[j].status.abstractGameState;
        var detailedState = cache_data.dates[0].games[j].status.detailedState;

        var scheduledInnings = await session.get_scheduled_innings(
          cache_data.dates[0].games[j]
        );
        ...
        if (
          ...
          if (
            inning_number != VALID_INNING_NUMBER[0] &&
            cache_data.dates[0].games[j].linescore &&
            cache_data.dates[0].games[j].linescore.innings
          ) {
            awayscore = 0;
            homescore = 0;
            let display_inning = "";
            for (
              var k = 0;
              k < cache_data.dates[0].games[j].linescore.innings.length;
              k++
            ) {
              if (cache_data.dates[0].games[j].linescore.innings[k]) {
                if (
                  cache_data.dates[0].games[j].linescore.innings[k].num <
                  relative_inning
                ) {
                  display_inning =
                    "T" + cache_data.dates[0].games[j].linescore.innings[k].num;
                  if (
                    typeof cache_data.dates[0].games[j].linescore.innings[k]
                      .away.runs !== "undefined"
                  )
                    awayscore +=
                      cache_data.dates[0].games[j].linescore.innings[k].away
                        .runs;
                  if (
                    typeof cache_data.dates[0].games[j].linescore.innings[k]
                      .home.runs !== "undefined"
                    ...
                    display_inning =
                      ...
                      cache_data.dates[0].games[j].linescore.innings[k].num;
                    homescore +=
                      cache_data.dates[0].games[j].linescore.innings[k].home
                        .runs;
                  ...
                  inning_half == VALID_INNING_HALF[2] &&
                  cache_data.dates[0].games[j].linescore.innings[k].num ==
                    relative_inning
                ) {
                  if (
            ...
            if (abstractGameState == "Live") {
              state =
                "<br/>" +
                cache_data.dates[0].games[j].linescore.inningHalf.substr(0, 1) +
                cache_data.dates[0].games[j].linescore.currentInning;
        ...
        if (
          cache_data.dates[0].games[j].resumeGameDate ||
          cache_data.dates[0].games[j].resumedFromDate
        ) {
          state += "<br/>Resum";
          let resumeDate;
          ...
          state +=
            "<br/>" +
            resumeDate.toLocaleString("default", {
              month: "long",
              day: "numeric",
            });
          ...
          if (
            typeof cache_data.dates[0].games[j].content.media != "undefined" &&
            typeof cache_data.dates[0].games[j].content.media.epg != "undefined"
          ) {
            for (
              var k = 0;
              k < cache_data.dates[0].games[j].content.media.epg.length;
              k++
            ) {
              if (cache_data.dates[0].games[j].content.media.epg[k].items) {
                for (
                  var x = 0;
                  x <
                  cache_data.dates[0].games[j].content.media.epg[k].items
                    .length;
                  x++
                ) {
                  if (
                    cache_data.dates[0].games[j].content.media.epg[k].items[x]
                      .mediaState &&
                    cache_data.dates[0].games[j].content.media.epg[k].items[x]
                      .mediaState == "MEDIA_ON"
                  ) {
        ...
        // Pitcher Info
        if (
          (cache_data.dates[0].games[j].teams["away"].probablePitcher &&
            cache_data.dates[0].games[j].teams["away"].probablePitcher
              .fullName) ||
          (cache_data.dates[0].games[j].teams["home"].probablePitcher &&
            cache_data.dates[0].games[j].teams["home"].probablePitcher.fullName)
        ) {
          pitchers = "<br/>";
          if (
            cache_data.dates[0].games[j].teams["away"].probablePitcher &&
            cache_data.dates[0].games[j].teams["away"].probablePitcher.fullName
          ) {
            if (
              cache_data.dates[0].games[j].teams["away"].team.sport.name !=
              "Major League Baseball"
        ...
        body += `<div class="w-full md:w-1/2 p-2" `;
        let fav_style = "";
        if (
          argv.free &&
          cache_data.dates[0].games[j].content &&
          cache_data.dates[0].games[j].content.media &&
          cache_data.dates[0].games[j].content.media.freeGame
        ) {
          body += ' class="freegame"';
        } else if (
          session.credentials.fav_teams.includes(
            cache_data.dates[0].games[j].teams["away"].team.abbreviation
          ) ||
          session.credentials.fav_teams.includes(
            cache_data.dates[0].games[j].teams["home"].team.abbreviation
          ...
          fav_style =
            ' style="color:#' +
            TEAM_COLORS[fav_team][0] +
            ";background:#" +
            TEAM_COLORS[fav_team][1] +
            ';"';
          body += fav_style;
        }
        let description = "";
        if (
          cache_data.dates[0].games[j].seriesDescription != "Regular Season"
        ) {
          description += cache_data.dates[0].games[j].seriesDescription + ": ";
        }
        // TODO: TD For game info
        body += `>
        <div class="shadow-md p-4" style="background-color:#171717;border-radius:4px;min-width:368px; min-height:176px">  
        <span>${description}  ${teams}  ${pitchers}  ${state}</span>`;
        ...
        if (
          ...
          if (cache_data.dates[0].games[j].broadcasts) {
            ...
            for (
              var k = 0;
              k < cache_data.dates[0].games[j].broadcasts.length;
              k++
            ) {
            ...
            if (broadcastName == "N/A") {
              ...
              if (
                cache_data.dates[0].games[j].status.detailedState !=
                  "Postponed" &&
                cache_data.dates[0].games[j].status.detailedState != "Cancelled"
              ) {
                // Check if game should be live
                ...
                if (currentTime >= startTime) {
                  ...
                  if (resumeStatus == false) {
                    if (inning_half != VALID_INNING_HALF[0])
                      querystring += "&inning_half=" + inning_half;
                    if (inning_number != VALID_INNING_NUMBER[0])
                      querystring += "&inning_number=" + relative_inning;
                  ...
                  if (linkType == VALID_LINK_TYPES[1]) {
                    let endTime = new Date(
                      cache_data.dates[0].games[j].gameDate
                    );
                  ...
                  body +=
                    "<button class='w-full bg-blue-500 text-white rounded' onclick=\"window.location.href='" +
                    thislink +
                    querystring +
                    "'\">" +
                    broadcastName +
          ...
          if (
            ...
            // TODO: start of game stream link
            body +=
              "<br/><span style='display: flex;flex-direction: column;gap: 6px;'>";
            for (
              var k = 0;
              k < cache_data.dates[0].games[j].content.media.epg.length;
              k++
              ...
              let mediaTitle =
                cache_data.dates[0].games[j].content.media.epg[k].title;
              if (mediaTitle == mediaType) {
                // initial loop will count number of broadcasts
                let broadcast_count = await session.count_broadcasts(
                  cache_data.dates[0].games[j].content.media.epg[k].items,
                  mediaType,
                  mediaTitle,
                  language
                ...
                if (cache_data.dates[0].games[j].content.media.epg[k].items) {
                  for (
                    ...
                    if (
                      typeof cache_data.dates[0].games[j].content.media.epg[k]
                        .items[x].language == "undefined" ||
                      cache_data.dates[0].games[j].content.media.epg[k].items[x]
                        .language == language
                    ) {
                      let teamabbr;
                      ...
                      if (
                        (typeof cache_data.dates[0].games[j].content.media.epg[
                          k
                        ].items[x].mediaFeedType != "undefined" &&
                          cache_data.dates[0].games[j].content.media.epg[k]
                            .items[x].mediaFeedType == "NATIONAL") ||
                        (mediaType == "MLBTV" &&
                          cache_data.dates[0].games[j].seriesDescription !=
                            "Regular Season" &&
                          cache_data.dates[0].games[j].seriesDescription !=
                            "Spring Training")
                      ) {
                      ...
                      if (blackouts[gamePk]) {
                        body +=
                          '<span class="tooltip"><span class="blackout">' +
                          teamabbr +
                          '</span><span class="tooltiptext">' +
                          blackouts[gamePk].blackout_type +
                          " video blackout until approx. 2.5 hours after the game";
                        if (blackouts[gamePk].blackoutExpiry) {
                          body +=
                            " (~" +
                            blackouts[gamePk].blackoutExpiry.toLocaleString(
                              "en-US",
                              {
                                hour: "numeric",
                                minute: "numeric",
                        ...
                        body +=
                          '<span class="tooltip">' +
                          teamabbr +
                          '<span class="tooltiptext">Regional FOX game</span></span>';
                      } else {
                        body += teamabbr;
                      }
                      body += ": ";
                      ...
                      if (
                        ...
                        if (
                          ...
                          gameDate == today &&
                          session.cache.media &&
                          session.cache.media[mediaId] &&
                          session.cache.media[mediaId].blackout &&
                          session.cache.media[mediaId].blackoutExpiry &&
                          new Date(
                            session.cache.media[mediaId].blackoutExpiry
                          ) > new Date()
                        ) {
                          body +=
                            '<span class="blackout">' + station + "</span>";
                        } else {
                          let querystring;
                          ...
                          if (mediaType == "MLBTV" && broadcast_count == 1) {
                            let alternateAudioTracks =
                              await session.getAlternateAudioTracks(
                                cache_data.dates[0].games[j].content.media.epg,
                                cache_data.dates[0].games[j].content.media.epg[
                                  k
                                ].items[x].mediaFeedType
                              );
                            for (const [key, value] of Object.entries(
                          ...
                          if (mediaType == "MLBTV") {
                            ...
                            if (audio_track != VALID_AUDIO_TRACKS[0])
                              querystring +=
                                "&audio_track=" +
                                encodeURIComponent(audio_track);
                          }
                          if (pad != VALID_PAD[0]) querystring += "&pad=" + pad;
                          if (linkType == VALID_LINK_TYPES[1]) {
                            if (
                              cache_data.dates[0].games[j].content.media.epg[k]
                                .items[x].mediaState == "MEDIA_ON"
                            ) {
                              if (force_vod != VALID_FORCE_VOD[0])
                                querystring += "&force_vod=" + force_vod;
                            }
                          ...
                          if (mediaType == "MLBTV") {
                            // TODO: currently commented checkbox for multiview
                            // body +=
                            //   '<input type="checkbox" value="' +
                            //   server +
                            //   "/stream.m3u8" +
                          ...
                          if (resumeStatus) {
                            ...
                            if (resumeStatus == "live") {
                              if (
                                cache_data.dates[0].games[j].content.media.epg[
                                  k
                                ].items[x].mediaState == "MEDIA_ARCHIVE"
                              ) {
                                body += "1";
                              } else {
                                body += "2";
                              ...
                              airings_data = await session.getAiringsData(
                                "",
                                cache_data.dates[0].games[j].gamePk
                              );
                              if (
                                airings_data.data &&
                                airings_data.data.Airings &&
                                airings_data.data.Airings.length > 0
                              ) {
                                for (
                                  var y = 0;
                                  y < airings_data.data.Airings.length;
                                  y++
                                ) {
                                  if (
                                    airings_data.data.Airings[y].contentId ==
                                    cache_data.dates[0].games[j].content.media
                                      .epg[k].items[x].contentId
                                  ) {
                                    if (
                                      (cache_data.dates[0].games[j]
                                        .resumeDate &&
            ...
            if (mediaType == "MLBTV" && game_started) {
              body +=
                `<button class="w-full bg-blue-500 text-white rounded"` +
                fav_style +
    ...
    if (argv.free) {
      body +=
        '<span class="freegame tooltip tinytext">green indicates a free game<span class="tooltiptext">Free games are available to anyone with an account, no subscription necessary. Blackouts still apply.</span></span>' +
        "\n";
    }
    ...
    if (mediaType == VALID_MEDIA_TYPES[0]) {
      ...
      for (var i = 0; i < VALID_RESOLUTIONS.length; i++) {
        ...
        if (DISPLAY_BANDWIDTHS[i] != "") {
          body +=
            '<br/><span class="tinytext">' + DISPLAY_BANDWIDTHS[i] + "</span>";
        }
        body += "</button> ";
      }
      body += "</p>" + "\n";
    ...
    body +=
      '<p><span class="tooltip">Pad<span class="tooltiptext">You can pad archive streams with random extra time at the end, to help conceal timeline spoilers.</span></span>: ';
    ...
    if (linkType == VALID_LINK_TYPES[1] && gameDate == today) {
      ...
      body +=
        '<span class="tinytext">(if client does not support seeking in live streams)</span></p>' +
        "\n";
    }
    ...
    body +=
      ' <span class="tinytext">(ON plays sample for all stream requests)</span></p>' +
      "\n";
    ...
    body +=
      '<p><span class="tooltip">All<span class="tooltiptext">Will include all live MLB broadcasts. If favorite team(s) have been provided, it will also include affiliate games for those organizations. If a zip code has been provided, channels/games subject to blackout will be omitted by default. See below for an additional option to override that.</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&resolution=" +
      resolution +
      content_protect_b +
      '">channels.m3u</a> and <a href="/guide.xml?mediaType=' +
      mediaType +
      content_protect_b +
      '">guide.xml</a></p>' +
      "\n";
    ...
    body +=
      '<p><span class="tooltip">Include blackouts<span class="tooltiptext">An optional parameter added to the URL will include channels/games subject to blackout (although you may not be able to play those games).</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&resolution=" +
      resolution +
      "&includeTeams=" +
    ...
    body +=
      '<p><span class="tooltip">Include (or exclude) LIDOM<span class="tooltiptext">Dominican Winter League, aka Liga de Beisbol Dominicano. Live stream only, does not support starting from the beginning or certain innings, skip options, etc.</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&resolution=" +
      resolution +
      "&includeTeams=lidom" +
    ...
    body +=
      '<p><span class="tooltip">Include (or exclude) Big Inning<span class="tooltiptext">Big Inning is the live look-in and highlights show. <a href="https://www.mlb.com/live-stream-games/big-inning">See here for more information</a>.</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&resolution=" +
      resolution +
      "&includeTeams=biginning" +
      content_protect_b +
      '">channels.m3u</a> and <a href="/guide.xml?mediaType=' +
      mediaType +
      "&includeTeams=biginning" +
    ...
    body +=
      '<p><span class="tooltip">Include (or exclude) Game Changer<span class="tooltiptext">The game changer stream will automatically switch between the highest leverage active live non-blackout games, and should be available whenever there are such games available. Does not support adaptive bitrate switching, will default to best resolution if not specified.</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&resolution=" +
      gamechanger_resolution +
      "&includeTeams=gamechanger" +
      content_protect_b +
      '">channels.m3u</a> and <a href="/guide.xml?mediaType=' +
      mediaType +
      "&includeTeams=gamechanger" +
    ...
    body +=
      '<p><span class="tooltip">Include (or exclude) Multiview<span class="tooltiptext">Requires starting and stopping the multiview stream from the web interface.</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&includeTeams=multiview" +
      content_protect_b +
      '">channels.m3u</a> and <a href="/guide.xml?mediaType=' +
      mediaType +
      "&includeTeams=multiview" +
    ...
    if (argv.free) {
      body +=
        '<p><span class="tooltip">Free games only<span class="tooltiptext">Only includes games marked as free. Blackouts still apply. If a zip code has been provided, channels/games subject to blackout will be omitted by default.</span></span>: <a href="/channels.m3u?mediaType=' +
        mediaType +
        "&resolution=" +
        resolution +
        "&includeTeams=free" +
    ...
    body +=
      '<p><span class="tooltip">Include affiliates by org<span class="tooltiptext">Including an organization (by MLB team abbreviation, in a comma-separated list if more than 1) will include all of its affiliate broadcasts, or if that affiliate is not broadcasting the game, it will include the opponent\'s broadcast if available. If this option is not specified, but favorite team(s) have been provided, affiliate games for those organizations will be included anyway.</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&resolution=" +
      resolution +
      "&includeOrgs=ari,atl" +
    ...
    body +=
      '<p><span class="tooltip">Include by level<span class="tooltiptext">Including a level (AAA, AA, A+, or A, in a comma-separated list if more than 1) will include all of its broadcasts, and exclude all other levels.</span></span>: <a href="/channels.m3u?mediaType=' +
      mediaType +
      "&resolution=" +
      resolution +
      "&includeLevels=aaa,aa" +
      content_protect_b +
      '">channels.m3u</a> and <a href="/guide.xml?mediaType=' +
      mediaType +
      "&includeLevels=aaa,aa" +
    ...
    body += "<table><tr><td>" + "\n";
    body +=
      '<p><span class="tooltip">Example links<span class="tooltiptext">Some examples how to generate predictable links.</span></span>:</p>' +
      "\n";
    body += "<p>" + "\n";
    let example_types = [
      ["embed.html", "Embed"],
      ["stream.m3u8", "Stream"],
      ["chromecast.html", "Chromecast"],
      ["kodi.strm", "Kodi"],
    ];
    ...
    let examples = [
      ...
      [
        "Catch-up/condensed",
        "?team=" + example_team + "&resolution=best&skip=pitches&date=today",
      ],
      [
        "Condensed yesterday",
        "?team=" +
          example_team +
          "&resolution=best&skip=pitches&date=yesterday",
      ],
      [
        "Same but DH game 2",
        "?team=" +
          example_team +
          "&resolution=best&skip=pitches&date=yesterday&game=2",
      ],
      ["Nat'l game 1 today", "?team=NATIONAL.1&resolution=best&date=today"],
      [
        "Same but incl. blackouts",
        "?team=NATIONAL.1&resolution=best&includeBlackouts=true",
      ],
      [
        "Nat'l game 2 yesterday",
        "?team=NATIONAL.2&resolution=best&date=yesterday",
      ],
    ];
    ...
    if (argv.free) {
      examples = examples.concat([
        ...
        [
          "Same but incl. blackouts",
          "?team=FREE.1&resolution=best&includeBlackouts=true",
        ],
        [
          "Free game 2 yesterday",
          "?team=FREE.2&resolution=best&date=yesterday",
        ],
      ]);
    }
    ...
    body +=
      '<p><span class="tooltip">Game Changer by team examples<span class="tooltiptext">Game Changer supports specifying certain teams to include or exclude. Useful for following a group of teams.</span></span>:</p>' +
      "\n";
    body += "<p>" + "\n";
    let gamechanger_streamURL =
      server + "/gamechanger.m3u8?resolution=best" + content_protect_b;
    let gamechanger_types = ["in", "ex"];
    for (var i = 0; i < gamechanger_types.length; i++) {
      let example_streamURL =
        gamechanger_streamURL +
        "&" +
        gamechanger_types[i] +
        "cludeTeams=ARI,ATL";
      body +=
        "&bull; " +
        gamechanger_types[i] +
        'clude: <a href="/embed.html?src=' +
        encodeURIComponent(example_streamURL) +
        "&startFrom=" +
        VALID_START_FROM[1] +
        content_protect_b +
        '">Embed</a> | <a href="' +
        ...
        encodeURIComponent(example_streamURL) +
        content_protect_b +
        '">Advanced</a> | <a href="/kodi.strm?src=' +
        encodeURIComponent(example_streamURL) +
        content_protect_b +
        '">Kodi</a><br/>' +
        "\n";
    ...
    body +=
      '<div id="myModal" class="modal"><div class="modal-content"><span class="close">&times;</span><div id="highlights"></div></div></div>';
...
app.get("/live-stream-games*", async function (req, res) {
  ...
  body = body.replace(
    /https:\/\/www.mlb.com\/tv\/g(\d+)\/[v]([a-zA-Z0-9-]+)/g,
    local_url + "&gamePk=$1&contentId=$2"
  );
...
app.get("/advanced.html", async function (req, res) {
  ...
  res.redirect(
    "http://hls-js-dev.netlify.app/demo/?src=" + encodeURIComponent(video_url)
  );
});
...
app.get("/channels.m3u", async function (req, res) {
  ...
  if (req.query.includeLevels) {
    includeLevels = decodeURIComponent(
      req.query.includeLevels.toUpperCase()
    ).split(",");
  }
  ...
  var body = await session.getTVData(
    "channels",
    mediaType,
    includeTeams,
    excludeTeams,
...
app.get("/highlights", async function (req, res) {
  ...
  try {
    ...
    if (req.query.gamePk && req.query.gameDate) {
      highlightsData = await session.getHighlightsData(
        req.query.gamePk,
        req.query.gameDate
      );
    }
...
function start_multiview_stream(
  streams,
  sync,
  dvr,
  faster,
  ...
  try {
    ...
    for (var i = 0; i < audio_present.length; i++) {
      ...
      if (sync[audio_present[i]]) {
        if (sync[audio_present[i]] > 0) {
          session.log(
            "delaying audio for stream " +
              (audio_present[i] + 1) +
              " by " +
              sync[audio_present[i]] +
              " seconds"
          );
    ...
    if (stream_count > 1) {
      ...
      ffmpeg_command
        .addOutputOption("-c:v", ffmpegEncoder)
    ...
    if (ffmpegEncoder == defaultEncoder) {
      ...
      ffmpeg_command
        .addOutputOption("-num_output_buffers", "32")
    ...
    ffmpeg_command
      ...
      .addOutputOption(
        ...
        session.get_multiview_directory() + "/stream_%v_%d.ts"
      )
